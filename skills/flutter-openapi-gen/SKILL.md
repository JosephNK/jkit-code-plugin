...
2. **Check/install npm dependencies** (plugin scripts require `yaml`, `nunjucks`):
   ```bash
   (cd ${CLAUDE_PLUGIN_ROOT} && { [ -d node_modules/yaml ] && [ -d node_modules/nunjucks ]; } || npm install)
   ```
   - `${CLAUDE_PLUGIN_ROOT}`가 `~/.pub-cache/git/...`처럼 fresh checkout인 경우 `node_modules`가 없어 후속 node 스크립트가 실패한다. 누락 시 자동 설치한다.
