...
- `E2` (error) — `entities/`/`ports/`/`exceptions/`만 import 허용 — adapters/presentation 금지 (bloc stack 활성 시 bloc 추가).
...
- `E3` (error) — bloc/은 `usecases`/`entities`/`exceptions`만 import — 외부는 `blocAllowedPackages` ∪ `leafKitBlocAllowed`만 허용.
...
- `E8` (error) — presentation/은 `adapters`/`ports` 직접 import 및 인프라 SDK import 금지 (bloc stack 활성 시 `usecases`도 차단 → bloc 경유 강제).
...
base 룰 12개 + stack 룰 1(`bloc`). stack 룰은 `analysis_options.yaml`의 `architecture_lint.stacks`에 해당 키가 포함될 때만 활성. 시각화된 의존 다이어그램은 `lint-rules-diagram.md` 참조.
...
| E2 | error | `usecases` | `entities/`/`ports/`/`exceptions/`만 import 허용 — adapters/presentation 금지 (bloc stack 활성 시 bloc 추가). | — |
...
| E6 | error | (all features) | feature 간 cross-import는 `entities/`와 다른 feature `domain/`만 허용. | `crossFeatureForbidden`, `crossFeatureForbiddenBloc` |
| E7 | error | (all) | bare `catch` 금지 — `on ExceptionType catch (e)` 형태 강제. | — |
| E8 | error | `presentation` | presentation/은 `adapters`/`ports` 직접 import 및 인프라 SDK import 금지 (bloc stack 활성 시 `usecases`도 차단 → bloc 경유 강제). | `infraPackages` |
...
`stacks: [bloc]` 활성 시 추가로 적용. 비활성 시 해당 룰은 합성 단계에서 제외된다.

| ID | Severity | Layer | 설명 | 참조 |
| --- | --- | --- | --- | --- |
| E3 | error | `bloc` | bloc/은 `usecases`/`entities`/`exceptions`만 import — 외부는 `blocAllowedPackages` ∪ `leafKitBlocAllowed`만 허용. | `blocAllowedPackages`, `leafKitBlocAllowed` |
