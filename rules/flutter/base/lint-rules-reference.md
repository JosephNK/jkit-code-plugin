...
- `E1` (error) — `entities/`는 codegen annotation 패키지만 import 허용.
...
- `E5` (error) — `ports/`는 framework 패키지 import 금지.
- `N1` (warning) — `ports/` 레이어 클래스는 `Port` suffix 강제 (warning).
...
- `E2` (error) — `usecases/`는 `entities/`, `ports/`, `exceptions/`만 import 허용.
- `N3` (warning) — `usecases/` 레이어 클래스는 `UseCase` 또는 `Params` suffix 강제 (warning).
...
- `N2` (warning) — `adapters/` 레이어 클래스는 `Adapter` suffix 강제 (warning).
...
- `E3` (error) — `bloc/`은 `usecases/`/`entities/`/`exceptions/`만 import 허용
...
| --- | --- | --- | --- | --- |
| E1 | error | `entities` | `entities/`는 codegen annotation 패키지만 import 허용. | `codegenPackages` |
| E2 | error | `usecases` | `usecases/`는 `entities/`, `ports/`, `exceptions/`만 import 허용. | — |
| E3 | error | `bloc` | `bloc/`은 `usecases/`/`entities/`/`exceptions/`만 import 허용 | `blocAllowedPackages` |
| E4 | error | `domainLayers` | 도메인 레이어(`domainLayers`)는 인프라 SDK import 금지. | `domainLayers`, `infraPackages` |
| E5 | error | `ports` | `ports/`는 framework 패키지 import 금지. | `frameworkPackages`, `infraPackages` |
| E6 | error | (all features) | feature 간 cross-import는 `entities/`와 다른 feature `domain/`만 허용. | `crossFeatureForbidden` |
| E7 | error | (all) | bare `catch` 금지 — `on ExceptionType catch (e)` 강제. | — |
| N1 | warning | `ports` | `ports/` 레이어 클래스는 `Port` suffix 강제 (warning). | — |
| N2 | warning | `adapters` | `adapters/` 레이어 클래스는 `Adapter` suffix 강제 (warning). | — |
| N3 | warning | `usecases` | `usecases/` 레이어 클래스는 `UseCase` 또는 `Params` suffix 강제 (warning). | — |
| S1 | warning | (all) | 파일당 최대 `maxFileLines`줄 (warning) — SRP 위반 신호. | `maxFileLines` |
