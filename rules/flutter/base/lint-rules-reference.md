...
- `E8` (error) — presentation/은 `adapters`/`ports`/`usecases` 직접 import 및 인프라 SDK import 금지 — bloc 경유 강제.
...
| E8 | error | `presentation` | presentation/은 `adapters`/`ports`/`usecases` 직접 import 및 인프라 SDK import 금지 — bloc 경유 강제. | `infraPackages` |
