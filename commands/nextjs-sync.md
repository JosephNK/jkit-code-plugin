...
Next.js 프로젝트의 JKit docs(`GIT.md`, `ARCHITECTURE.md`, `CONVENTIONS.md`, `LINT.md`)와 lint config(`eslint.config.mjs`, `stylelint.config.mjs`)를 플러그인 최신 버전과 동기화합니다.

> 이 커맨드는 init이 아닙니다. `AGENTS.md`, `AGENTS.LOCAL.md`, `CONVENTIONS.LOCAL.md`, `tsconfig.json`, `.husky/`, `commitlint.config.mjs`는 건드리지 않습니다. 최초 셋업은 `/jkit-nextjs-init`를 사용하세요.
...
if [ -n "$USER_CONV_STACKS" ]; then
  $JKIT_DIR/scripts/gen-conventions.mjs nextjs -p docs --with "$USER_CONV_STACKS" --no-local-init
else
  $JKIT_DIR/scripts/gen-conventions.mjs nextjs -p docs --no-local-init
fi
