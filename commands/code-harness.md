...
> 사용자가 작성한 TASKS.md(task-source)·QA.md(eval-source)를 정본으로 Task 단위 구현-검증을 실행한다. **1회 실행 = 1라운드**, 루프 기본 활성(ScheduleWakeup 자동 재개). `--once`로 단일 라운드 모드.

## Architecture

```
TASKS.md ─┐
QA.md  ───┴→ Generator(jkit:code-generator)
             → Gate 1 코드 리뷰 (codex → ecc:{stack}-reviewer fallback → SKIP)
             → Gate 2 린트·빌드
...
|---|---|---|
| Flutter/Dart | `pubspec.yaml` | `dart format . && dart analyze` (조건부 `&& dart run architecture_lint:lint`) |
...
> **Flutter architecture_lint 조건부**: 임의의 `pubspec.yaml`의 `dependencies`/`dev_dependencies`에 `architecture_lint` 키(`grep -q '^\s*architecture_lint:' pubspec.yaml`)가 있으면 명령 뒤에 `&& dart run architecture_lint:lint`를 덧붙인다.

#### codex CLI 감지

`which codex`로 설치 여부를 확인하고 결과를 `codexAvailable`에 저장한다.
...
> Gate 1 FAIL 후 PASS된 교훈 중 lint 자동 검출 불가(`Automatable: unlikely | no`) 항목을 축적. Generator가 매 라운드 읽어 재발 방지 — 토큰 최소화를 위해 메타데이터 미기록. `likely` 항목은 `docs/LEARNED-LINT.md`(Generator 미열람).
...
> Gate 1 FAIL 후 PASS된 교훈 중 lint 자동 검출 가능(`Automatable: likely`) 항목을 축적. `rules/{stack}/custom-lint/` 룰 승격 대기열 — 사람이 주기 검토로 실제 lint 룰로 이관하고 승격된 항목은 삭제. Generator는 읽지 않음.
