...
|---|---|---|
| Flutter/Dart | `pubspec.yaml` | `dart format . && dart analyze` (조건부 `&& dart run custom_lint`) |
...
> **Flutter custom_lint 조건부**: 임의의 `pubspec.yaml`의 `dependencies`/`dev_dependencies`에 `custom_lint` 키(`grep -q '^\s*custom_lint:' pubspec.yaml`)가 있으면 명령 뒤에 `&& dart run custom_lint`를 덧붙인다. custom_lint이 architecture_lint 및 설치된 stack lint 패키지를 자동 발견·합성한다.
