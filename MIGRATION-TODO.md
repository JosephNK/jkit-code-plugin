...
| `openapi/` | 5 | `__init__.py`, `_run.py`, `dart_name_utils.py` 🔄 (dart-name-utils.mjs 존재; .py는 generator 이전 후 삭제), `generate_api.py`, `openapi_parser.py` 🔄 (openapi-parser.mjs 존재; .py는 generator 이전 후 삭제), ~~`update_pubspec.py`~~ ✅ (update-pubspec.mjs) |
