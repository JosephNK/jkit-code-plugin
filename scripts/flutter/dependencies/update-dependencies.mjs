...
async function analyzeDependencies(pubspecPath, excludePrefixes) {
  ...
  for (const section of ["dependencies", "dev_dependencies"]) {
    ...
    for (const [pkgName, version] of Object.entries(deps)) {
      ...
      if (excludePrefixes.some((prefix) => pkgName.startsWith(prefix)))
        continue;
  ...
  for (const section of ["dependencies", "dev_dependencies"]) {
    ...
    for (const dep of analysis[section]) {
      ...
      const pair = sectionNode.items.find(
        (p) =>
          (YAML.isScalar(p.key) ? String(p.key.value) : String(p.key)) ===
          dep.name,
      );
      if (!pair) continue;
      ...
      updatedCount += 1;
      process.stdout.write(
        `  ✨ ${dep.name}: ${dep.current} -> ${newVersion}\n`,
      );
    }
  }
...
async function main() {
  ...
  process.stdout.write("\n");
  process.stdout.write(
    `📋 ${pubspecFiles.length}개의 패키지를 분석합니다...\n`,
  );
  ...
  process.stdout.write("\n");
  const answer = (await prompt("⚡ 업데이트를 진행하시겠습니까? [y/N]: "))
    .trim()
    .toLowerCase();
  if (answer !== "y") {
    process.stdout.write("\n⏭️  업데이트가 취소되었습니다.\n");
  ...
  process.stdout.write(`${"=".repeat(60)}\n`);
  process.stdout.write(
    `🎉 총 ${totalUpdated}개의 의존성이 업데이트되었습니다.\n`,
  );
  process.stdout.write(`${"=".repeat(60)}\n`);
}
