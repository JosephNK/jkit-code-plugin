...
// `{{PLACEHOLDER}}` 마커는 gen-eslint.mjs가 선택된 스택(Mantine, TanStack Query 등)의
...
} from "@jkit/code-plugin/nextjs/design-system/shadcn/eslint.rules.mjs";
import {
  nextauthBoundaryAllowPatches,
  nextauthBoundaryElements,
  nextauthBoundaryRules,
  nextauthDomainBannedPackages,
} from "@jkit/code-plugin/nextjs/nextauth/eslint.rules.mjs";
import { tanstackQueryDomainBannedPackages } from "@jkit/code-plugin/nextjs/tanstack-query/eslint.rules.mjs";
...
const eslintConfig = [
  ...
  ...buildArchitectureBoundaries(
    [...baseBoundaryElements, ...nextauthBoundaryElements],
    patchBoundaryRules(
      [...baseBoundaryRules, ...nextauthBoundaryRules],
      [...shadcnBoundaryAllowPatches, ...nextauthBoundaryAllowPatches],
    ),
    [...baseBoundaryIgnores],
  ),
